  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch((reason: unknown) => {
        setError(reason instanceof Error ? reason.message : t("dataError"));
      })
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: t("filesCount"), value: stats.totalFiles },
            { label: t("totalSize"), value: formatFileSize(stats.totalSize) },
            { label: t("activeBots"), value: stats.activeAccounts },
            { label: t("expiredCount"), value: stats.expiredFiles },
          ].map((stat) => (
        ...
        {[
          {
            ...
            description: t("usersSectionDescription"),
          },
          {
            href: "/admin/files",
            icon: "📁",
            title: t("filesSection"),
            description: t("filesSectionDescription"),
          },
          {
            href: "/admin/bots",
            icon: "🤖",
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="text-gray-600 transition-colors group-hover:text-accent-light">
                →
              </span>
            </div>
      ...
      <div className="glass rounded-2xl p-5">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            <h2 className="font-medium">{t("recentActions")}</h2>
            <p className="text-xs text-gray-500 mt-1">
              {t("auditDescription")}
            </p>
          </div>
          <span className="text-xs text-gray-500">{t("latest10")}</span>
        </div>
        {auditEvents.length > 0 ? (
          <div className="space-y-2 text-sm">
            {auditEvents.map((event) => (
              <div
                key={event.id}
                className="flex flex-wrap gap-x-3 gap-y-1 border-b border-white/5 pb-2 text-gray-400 last:border-0 last:pb-0"
              >
